...
def get_http_proxy():
    ...
    try:
        ...
        response = requests.get(proxy_url, impersonate="chrome110", timeout=30)
...
def get_proxyscrape_proxies():
    proxies = requests.get(
        "https://api.proxyscrape.com/v4/free-proxy-list/get?request=display_proxies&country=bd&proxy_format=protocolipport&format=text",
        impersonate="chrome110",
        timeout=30,
    ).text.split("\n")
    proxies = [proxy.strip() for proxy in proxies if proxy.strip()]
